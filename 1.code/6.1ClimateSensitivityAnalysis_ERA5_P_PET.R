...
# This script performs the driver/sensitivity analysis in Figure D5 (using ERA5 P and PET).
...
WeatherData<-readRDS("2.data/2.working/WeatherDataERA5Land/WeatherData.RDS")
...
for(it_stn in 1:length(stations$ID)){
  ...
  dataYearly<-data%>%
    ...
    dplyr::summarize(
      ...
      winterPrecip = sum(pcp[Month%in%c(11,12,1,2,3,4)],na.rm = TRUE),
      meanWinterTemp =mean(temp[Month%in%c(11,12,1,2,3,4)],na.rm = TRUE),
      
      summerPrecip = sum(pcp[Month%in%c(5:stations$minSumFlowMonth[it_stn])],na.rm = TRUE),
      meanSummerET = mean(pet[Month%in%c(5:stations$minSumFlowMonth[it_stn])],na.rm = TRUE)
      
      # Precip_MJJA = sum(pcp[Month%in%c(5,6,7,8)],na.rm = TRUE),
  ...
  dat<-dataYearly2[,c("minSumFlow7.log","summerPrecip","meanSummerET","SumT7","maxSWEdly",
  ...
  summary(mylm<-lm(minSumFlow7.log~summerPrecip+meanSummerET+SumT7+
...
stnCoeffs$meanSummerET<- (-1*as.numeric(stnCoeffs$meanSummerET))%>%as.character()
...
stnCoeffs.long$variable<-factor(stnCoeffs.long$name,
                                levels = c("maxSWEdly","BF_30day_Eckhardt0.97","summerPrecip","meanSummerET","SumT7","Total.cms","ECA_I_9","ECA_III_24"),
                                labels = c("SWE[max]","BF[winter]","P[summer]","PET[summer]","T[7]","Abstraction","ECA[I]", "ECA[III]"))
...
ggsave("3.figures/Sens_boxplots_beta_v2.png",width = 4, height = 7.5)

...
# This script performs the driver/sensitivity analysis in Figure D6 (using PNWNAmet data)
...
WeatherDataDaily<-readRDS("2.data/2.working/WeatherDataPWNAmet/dataDaily.RDS")
...
for(it_stn in 1:length(stations$ID)){
  ...
  if(stations$ID[it_stn] %in% c("08OA004","08OA005","08HD023","08EE003")){
    dat<-dataYearly2[,c("minSumFlow7.log","summerPrecip","meanSummerTemp","SumT7","maxSWEdly",
...
ggsave("3.figures/Sens_boxplots_beta_PWNAmet_P_T.png",width = 4, height = 7.5)
...
WeatherData%>%
  filter(Month==8)%>%
  group_by(ID)%>%
  dplyr::summarize(cor_P_T = cor(pcp,temp),
                   cor_P_PET = cor(pcp,pet),
                   cor_P_ET = cor(pcp,et))%>%
  ungroup()%>%
  summary()
...
WeatherData%>%
  ...
  dplyr::summarize(cor_P_T = cor(pcp,temp),
                   cor_P_PET = cor(pcp,pet),
                   cor_P_ET = cor(pcp,et),
                   cor_ET_PET = cor(et,pet))%>%
  ungroup()%>%
  summary()

ggplot(WeatherData%>%filter(Month==8&ID=="08HA003"),
       aes(x = pcp,y = pet))+
  geom_point()
...
ggplot(WeatherData%>%filter(Month==7&ID=="08GA047"),
       aes(x = -pet,y = -et))+

...
# This script runs routine 'B' of the ECA analysis (Appendix E)
...
stn_MC<-stations%>%
  ...
  dplyr::select(ID,regime,Area_km2,Total.Cut.Area)
...
stnRs_long<-stnRs%>%
  st_drop_geometry()%>%
  dplyr::select(ID,regime,Total.Cut.Area,Area_km2,c0.60:c0.5.p)%>%
...
stnRs_long_lag1<-stnRs%>%
  st_drop_geometry()%>%
  dplyr::select(ID,regime,Total.Cut.Area,Area_km2,c1.60:c1.5.p)%>%

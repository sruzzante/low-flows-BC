...
# This script runs the power analysis of routines A, B, and C described in Appendix E
...
stn_MC_result<-foreach(stn_MC_x = stn_MC_ls,
  ...
  if(all(streamDataYrly$ECA_5==0)){return(NULL)}
...
for(it_fctr in 1:length(fctrs)){
  ...
  x2_glac<-apply(x[,stn_MC$regime=="Glacial"],1,binom.testfunc,0.05)
  ...
  type2Errors$H1_rate_glac[it_fctr]<- 1-(sum(x2_glac<0.05)/10000)
  ...
  x2_glac<-apply(x[,stn_MC$regime=="Glacial"],1,binom.testfunc,0.5)
  ...
  type2Errors$H2_rate_glac[it_fctr]<- 1-(sum(x2_glac<0.05)/10000)
  
}



type2Errors_2<-pivot_longer(type2Errors,cols = H1_rate_rain:H2_rate_glac)
type2Errors_2$hypo<-substr(type2Errors_2$name,1,2)
type2Errors_2$regime<-substr(type2Errors_2$name,9,12)%>%
  factor(levels = c("rain","hybr","snow",'glac'),
